"""
btc_auto.py — 自主策略優化迴圈
類似 autoresearch 的模式：隨機擾動參數 → 回測 → 保留或還原 → 無限迴圈。

使用方式:
    python btc_auto.py

按 Ctrl+C 停止。結果記錄在 btc_results.tsv。
主要指標：sharpe（越高越好）
"""
...
RESULTS_FILE  = os.path.join(os.path.dirname(__file__), "btc_results.tsv")
BACKUP_FILE   = STRATEGY_FILE + ".bak"
LOG_FILE      = os.path.join(os.path.dirname(__file__), "btc_run.log")
...
# 進出場邏輯組合（AND 邏輯）
ENTRY_COMBOS = [
    ["macd_bull", "rsi_exit_oversold", "price_breakout_zlsma"],   # S01 baseline
...
EXIT_COMBOS = [
    ["macd_bear", "rsi_exit_overbought"],             # S01
    ["rsi_exit_overbought"],                           # S03
    ["rsi_exit_overbought", "price_breakdown_zlsma"],  # S04
    ["rsi_exit_overbought", "consecutive2_day_down"],  # S06
    ["macd_bear"],
...
def read_full_metrics() -> dict:
    ...
    with open(LOG_FILE) as f:
        for line in f:
            for key in ["sharpe", "profit_factor", "win_rate", "max_drawdown_pct",
...
def describe_experiment(params_before: dict, params_after: dict,
                        entry_before: list, exit_before: list,
                        entry_after: list, exit_after: list) -> str:
    diffs = []
    for k in params_after:
        if params_before.get(k) != params_after[k]:
            diffs.append(f"{k}: {params_before.get(k)}→{params_after[k]}")
    if entry_before != entry_after:
        diffs.append(f"ENTRY: {entry_before}→{entry_after}")
    if exit_before != exit_after:
        diffs.append(f"EXIT: {exit_before}→{exit_after}")
...
def main():
    ...
    if baseline_sharpe is None:
        print("❌ Baseline 回測失敗，請確認數據已準備好（python btc_prepare.py）")
        sys.exit(1)
    ...
    while True:
        ...
        if new_sharpe is None:
            ...
            print(f"  ✅ 改善！sharpe = {new_sharpe:.4f} (+{improvement:.4f})  "
                  f"[{elapsed:.0f}s]")
            print(f"     profit_factor={metrics.get('profit_factor','?')}  "
                  f"win_rate={metrics.get('win_rate','?')}  "
                  f"trades={metrics.get('num_trades','?')}")
